...
use piccolo::{
    AnyCallback, AnyUserData, CallbackReturn, Context, Error, Execution, FromMultiValue, Fuel,
...
impl<'gc, U: for<'a> Rootable<'a>> UserMethods<'gc, U> {
    ...
    where
        F: Fn(&Root<'gc, U>, Context<'gc>, &Execution, A) -> Result<R, Error<'gc>> + 'static,
        A: FromMultiValue<'gc>,
        R: IntoMultiValue<'gc>,
    {
    ...
    where
        F: Fn(&barrier::Write<Root<'gc, U>>, Context<'gc>, &Execution, A) -> Result<R, Error<'gc>>
            + 'static,
        A: FromMultiValue<'gc>,
        R: IntoMultiValue<'gc>,
    {
...
impl<'gc, U: 'static> StaticUserMethods<'gc, U> {
    ...
    pub fn add<F, A, R>(self, name: &'static str, ctx: Context<'gc>, method: F) -> bool
    where
        F: Fn(&U, Context<'gc>, &Execution, A) -> Result<R, Error<'gc>> + 'static,
        A: FromMultiValue<'gc>,
        R: IntoMultiValue<'gc>,
    {
        let callback = AnyCallback::from_fn(&ctx, move |ctx, fuel, mut stack| {
